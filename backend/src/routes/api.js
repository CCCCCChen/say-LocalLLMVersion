...
router.post('/users/:userId/notes', upload.single('audio'), handleUploadError, async (req, res) => {
    try {
        ...
        // 如果有音频文件，更新笔记状态为转录中
        if (req.file) {
            await db.updateNote(note.id, { status: 'transcribing' });
...
router.put('/notes/:noteId', async (req, res) => {
    try {
        ...
        const updatedNote = await db.updateNote(noteId, updates);
...
async function processTranscription(noteId, audioFilePath, jobId) {
    try {
        console.log(`Starting transcription for note ${noteId}, job ${jobId}`);
        ...
        try {
            ...
            console.log('WebSocket转录失败，尝试HTTP方式:', wsError.message);
        ...
        console.log(`转录完成: job ${jobId}, method: ${transcriptionMethod}`);
        
        // 清理音频文件（可选）
        try {
            funasrService.cleanupAudioFile(audioFilePath);
        } catch (cleanupError) {
            console.warn('清理音频文件失败:', cleanupError.message);
        }
        
    } catch (error) {
        console.error(`转录失败 job ${jobId}:`, error.message);
        
        // 更新任务状态为失败
        await db.updateTranscriptionJobStatus(jobId, 'failed', error.message).catch(console.error);
        
        // 更新笔记状态为错误
        await db.updateNoteStatus(noteId, 'error').catch(console.error);
    }
}
